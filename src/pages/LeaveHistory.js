...
import { sickLeaveExceptionHandler } from '../utils/utils';
...
const leaveReducer = (curLeaveState, action) => {
    switch (action.type) {
        case ACTIONS.SET_SUMMARY:
            return { ...curLeaveState, summary: action.summary, summaryMap: (action.summaryMap ? action.summaryMap : curLeaveState.summaryMap) }
        case ACTIONS.SET_SUMMARY_MAP:
            return { ...curLeaveState, summaryMap: action.summaryMap }
        case ACTIONS.SET_SUMMARY_AND_MAP:
            return { ...curLeaveState, summary: action.summary, summaryMap: action.summaryMap }
        ...
        case ACTIONS.HANDLE_RESET:
            return { ...curLeaveState, summary: [], summaryMap: {}, selectedYear: "2021", order: "asc", orderBy: "submitted_date", showType: "total", deleteDialog: null }
        default:
...
export default function LeaveHistory(props) {
    ...
    const [{ summary, summaryMap }, dispatchLeave] = useReducer(leaveReducer,
        { summary: [], summaryMap: {}, selectedYear: "", order: "asc", orderBy: "submitted_date", showType: "total", deleteDialog: null, successDialog: {}, });
    ...
    const handleLeaveDelete = (id) => {
        ...
        handleBasicDialog(true, "Are you sure you want to cancel this leave?", () => {
            handleRequestWithNewToken(() => { handleRequest(`${services.CANCEL_LEAVE}/${id}`, 'DELETE', null, successFn, errorFunc, loadingFunc); });
        });
        // showConfirmAlert("Cancel Confirmation", "Are you sure you want to cancel this leave?", "Confirm", "Cancel", () => {
        //     handleRequestWithNewToken(() => { handleRequest(`${services.CANCEL_LEAVE}/${id}`, 'DELETE', null, successFn, errorFunc, loadingFunc); });
        // }, () => { showAlert("Error", "Error while cancelling leave", "Close", () => { }, () => { }); }, () => { });
    };

    const loadSummary = () => {
        setIsLoading(true);
        handleRequestWithNewToken(() => {
            handleRequest(`${services.LIST_LEAVE}?isActive=true&email=${getUserName()}&startDate=${getStartYear()}`, "GET", null,
                (data) => {
                    if (data) {
                        var summaryMap = {};
                        var summary = [];
                        if (data.stats && props.leaveMap) {
                            ...
                            summaryArray.forEach((stat) => {
                                let statElement = sickLeaveExceptionHandler(stat);
                                ...
                                if (summaryMap['all']) {
                                    summaryMap['all']['count'] = summaryMap['all']['count'] + statElement.count;
                        ...
                        if (data.leaves) {
                            ...
                            summary.forEach((leave) => {
                                ...
                                if (summaryMap[leaveElement.leaveType]) {
                                    ...
                                    summaryMap[leaveElement.leaveType]['list'] = summaryMap[leaveElement.leaveType]['list'].concat(leaveElement);
                                ...
                                if (summaryMap['all']) {
                                    ...
                                    summaryMap['all']['list'] = summaryMap['all']['list'].concat(leaveElement);
    ...
    useEffect(() => {
    }, [props.leaveMap, props.leaveYears])
    ...
    return (
        <>
            ...
            {isMobile ? <span>
                <HideOnScroll {...props}>
                    <AppBar sx={{ backgroundColor: 'transparent', width: '100%', paddingLeft: 0, paddingRight: 0 }} ref={targetRef}>
                        <Toolbar sx={{ paddingLeft: 0, paddingRight: 0, paddingBottom: 0, width: '100%' }}>
                            <LeaveSummarySelector leaveType={leaveType} handleLeaveType={handleLeaveType} leaveMap={summaryMap} collapsed={true} />
                        </Toolbar>
                    </AppBar>
                </HideOnScroll>
                <ShowOnScroll>
                    <AppBar elevation={0} sx={{ backgroundColor: 'transparent', width: '100%', paddingLeft: 0, paddingRight: 0 }}>
                        <Toolbar sx={{ paddingLeft: 0, paddingRight: 0, paddingBottom: 0, width: '100%' }}>
                            <LeaveSummarySelector leaveType={leaveType} handleLeaveType={handleLeaveType} leaveMap={summaryMap} collapsed={false} />
                        </Toolbar>
                    </AppBar>
                </ShowOnScroll>
            </span> :
            ...
            <Container sx={{ overflow: 'hidden', padding: 0 }}>
                <Box sx={{ my: 2 }}>
                    ...
                    <LeaveHistoryList leaveMap={summaryMap} leaveType={leaveType} leaves={summaryMap['all'] && summaryMap['all'].list ? summaryMap['all'].list : []} isLoading={isLoading} onDelete={handleLeaveDelete} />
                </Box>
            </Container>
            <ScrollTop {...props}>

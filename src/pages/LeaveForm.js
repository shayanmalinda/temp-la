...
//function to remove duplicates from array of recipients
...
const leaveReducer = (curLeaveState, action) => {
  switch (action.type) {
    ...
    case ACTIONS.FULL_DAY_LEAVE:
      return { ...curLeaveState, isFullDayLeave: action.isFullDayLeave, isMorningLeave: true };
    case ACTIONS.IS_MORNING_LEAVE:
      return { ...curLeaveState, isMorningLeave: action.isMorningLeave, isFullDayLeave: false };
    ...
    case ACTIONS.SET_EMAIL_RECIPIENTS:
      return { ...curLeaveState, emailRecipients: removeDuplicateRecipients(action.emailRecipients), validationAddPeople: action.validationAddPeople, ...(action.savedRecipients && { savedRecipients: action.savedRecipients }) }
...
const LeaveForm = (props) => {
  ...
  const { navigatedView } = useSelector((state) => state.menu);
  ...
  const [{ leaveMap, leaveType, isFullDayLeave, isMorningLeave, emailRecipients, defaultRecipients, savedRecipients, emailMap, searchEmail, validationAddPeople, successDialog, leaveTypes, leaveYears, comment, isPublicComment, }, dispatchForm] = useReducer(leaveReducer,
    { leaveMap: {}, leaveType: 'casual', isFullDayLeave: true, isMorningLeave: true, emailRecipients: [], defaultRecipients: [], savedRecipients: [], emailMap: {}, searchEmail: null, validationAddPeople: true, successDialog: {}, leaveTypes: [], leaveYears: [], comment: "", isPublicComment: true });
  ...
  const handleBasicDialog = (isOpen, message, callbackFn) => {
    dispatch(openBasicDialog({ openBasicDialog: isOpen, basicDialogMessage: message, basicDialogCallbackFn: callbackFn }));
  };
  ...
  const loadForm = () => {
    ...
    handleRequestWithNewToken(() => {
      handleRequest(services.LEAVE_FORM, "GET", null,
        (data) => {
          var leadEmails = [];
          if (data) {
            if (data.leaveTypes) {
            ...
            if (data.leadEmails) {
              leadEmails = Array.isArray(data.leadEmails) ? data.leadEmails.slice() : [];
            ...
            if (data.emailRecipients) {
              const emailRecipients = data.emailRecipients.filter((e) => {
                return ((!e.search(emailRegex) >= 0) && (e.length > 2) && !(leadEmails.find(email => email === e.trim())))
              }).map((e) => {
                return {
                  workEmail: e.trim(),
                };
              ...
              dispatchForm({
                ...
                emailRecipients: [...LEAVE_APP.DEFAULT_EMAIL_RECIPIENTS, ...defaultRecipients, ...emailRecipients],
        ...
        () => {
          showAlert("Error", "Error while loading form", "Close", () => { }, () => { });
        },
        () => {
        });
    });
  ...
  const loadWorkingDays = () => {
    ...
    const body = {
      "periodType": (amountOfDays > 1 ? "multiple" : isFullDayLeave ? "one" : "half"),
      "startDate": getDateFromDateString(startDate),
      "endDate": getDateFromDateString(endDate),
      "isMorningLeave": (amountOfDays > 1 ? null : isFullDayLeave ? null : isMorningLeave),
    };

    const successFn = (data) => {
      if (data) {
        handleWorkingDays((data.workingDays ? data.workingDays : 0), (data.hasOverlap ? data.hasOverlap : false));
    ...
    handleRequestWithNewToken(() => { handleRequest(`${services.CALCULATE_DAYS_OF_LEAVE}`, 'POST', body, successFn, errorFunc, loadingFunc); });
  };
  ...
  const loadEmployeeData = () => {
    ...
    handleRequestWithNewToken(() => { handleRequest(services.FETCH_EMPLOYEES, 'GET', null, successFn, errorFunc, loadingFunc); });
  };
  ...
  const handleSubmitForm = () => {
    ...
    const errorFunc = (error) => {
      setSuccess(false);
      showAlert("Error", "Error while submitting leave", "Close", () => { }, () => { });
    };
    ...
    handleBasicDialog(true, "Proceed with leave submission.", () => {
      ...
      handleRequestWithNewToken(() => { handleRequest(services.SUBMIT_LEAVE, 'POST', body, successFn, errorFunc, loadingFunc); });
    });
    // showConfirmAlert("Confirm Submission", "Proceed with leave submission.", "Confirm", "Cancel", () => {
    //   const body = {
    //     leaveType,
    //     periodType: (amountOfDays > 1 ? "multiple" : isFullDayLeave ? "one" : "half"),
    //     emailRecipients: emailRecipients.map(email => email.workEmail).join(','),
    //     startDate: getDateFromDateString(startDate),
    //     endDate: getDateFromDateString(endDate),
    //     isMorningLeave: (amountOfDays > 1 ? 0 : isFullDayLeave ? null : isMorningLeave),
    //     comment,
    //     isPublicComment,
    //   };
    //   handleRequest(services.SUBMIT_LEAVE, 'POST', body, successFn, errorFunc, loadingFunc);
    // }, () => { }, () => { });
  };
    ...
    if (email && email.workEmail) {
      ...
      if (index >= 0) {
        ...
        dispatchForm({
          ...
          validationAddPeople: (email.workEmail === (searchEmail) ? true : validationAddPeople)
        });
      }
    }
  };
  ...
  return (
    <>
      <TabPanel value={value} index={0} sx={{ padding: 2 }} dir={theme.direction}>
        <Grid container spacing={2}
          ...
          <Grid item sm={12} md={6}>
            ...
            <Grid item xs={12}>
              <DateRangePicker startDate={startDate} endDate={endDate} amountOfDays={amountOfDays} workingDays={workingDays} handleStartDate={handleStartDate} handleEndDate={handleEndDate} loadWorkingDays={loadWorkingDays}
                isLoading={isLoadingDates} isMorningLeave={isMorningLeave} isFullDayLeave={isFullDayLeave} errorForWorkingDays={errorForWorkingDays} hasOverlap={hasOverlap} isSubmitted={isSubmitted} />
            </Grid>
          </Grid>
          <Grid item sm={12} md={6}>
            <Stack
              ...
              <span style={{ width: '100%', alignSelf: 'center' }}>
                ...
                <Grid item xs={12}>
                  <Stack direction="row"
                    ...
                    alignItems="flex-start"
                    spacing={0}>
                    {Object.values(LEAVE_APP.LEAVE_TYPES).map((leave, index) => (
                      <Stack
                        sx={{ width: 100 }}
                        direction="column"
                        justifyContent="center"
                        alignItems="center"
                        spacing={0}>
                        <Avatar sx={{ backgroundColor: leaveType === leave.type && `${theme.palette.secondary.main} !important` }}>
                          <IconButton onClick={handleLeaveType(leave.type)} selected={leaveType === leave.type} >{leave.icon}
                          </IconButton>
                        </Avatar>
              ...
              <span style={{ width: '100%', alignSelf: 'center' }}>
                ...
                <Stack direction="row"
                  ...
                  <Button sx={{
                    ...
                    padding: 1,
                    border: '0.5px solid',
                    // color: isFullDayLeave && '#fff',
                    backgroundColor: isFullDayLeave && `${theme.palette.secondary.main} !important`,
                  }} onClick={handleFullDayLeave} selected={isFullDayLeave} variant="outlined" size="large">
                    Full Day
                  </Button>
                  <Button variant="outlined" size="large"
                    sx={{
                      width: 120,
                      background: `linear-gradient(to right bottom, ${(!isFullDayLeave && isMorningLeave) ? theme.palette.secondary.main : '#fff'} 50%, #fff 50%)`,
                      borderRadius: 4,
                      padding: 1,
                      border: '0.5px solid',
                    }}
                    ...
                    //   backgroundColor: (!isFullDayLeave && isMorningLeave) && `${theme.palette.secondary.main} !important`,
                    //   color: (!isFullDayLeave && isMorningLeave) && '#fff',
                    // }} 
                    disabled={amountOfDays > 1
                    } onClick={() => { handleIsMorningLeave(true) }} selected={(!isFullDayLeave && isMorningLeave)}>
                    First Half
                  </Button>
                  <Button variant="outlined" size="large" sx={{
                    width: 120,
                    background: `linear-gradient(to right bottom, #fff 50%, ${(!isFullDayLeave && !isMorningLeave) ? theme.palette.secondary.main : '#fff'} 50%)`,
                    borderRadius: 4,
                    padding: 1,
                    border: '0.5px solid',
                    ...
                    // backgroundColor: (!isFullDayLeave && !isMorningLeave) && `${theme.palette.secondary.main} !important`,
                    // color: (!isFullDayLeave && !isMorningLeave) && '#fff'
                  }} disabled={amountOfDays > 1} onClick={() => { handleIsMorningLeave(false) }} selected={(!isFullDayLeave && !isMorningLeave)}>
                    Second Half
                  </Button>
                </Stack>
              </span>
          ...
          <Grid container item xs={12}>
            ...
            <Grid item xs={12}>
              <NotifyPeople isLoading={isLoading} defaultRecipients={defaultRecipients} emailRecipients={emailRecipients} savedRecipients={savedRecipients}
          ...
          <Grid item xs={12} sx={{ display: 'flex', alignItems: 'center' }}>
            <Box sx={{ position: 'relative', width: '100%' }}>
              <Button
                ...
                variant="contained"
                sx={buttonSx}
                disabled={isLoading || isLoadingDates || isSubmitLoading || hasOverlap || errorForWorkingDays || workingDays <= 0}
                color="secondary"
